...
namespace SteamLink
{
    static class DisplayManager
    {
        ...
        {
            User32.DEVMODE mode = GetDeviceMode();
            ...
            DisplayChangeResult result = (DisplayChangeResult)User32.ChangeDisplaySettings(ref mode, 0);

            string msg = null;
            switch (result)
            {
                case DisplayChangeResult.BadDualView:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_BadDualView;
                    break;
                case DisplayChangeResult.BadParam:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_BadParam;
                    break;
                case DisplayChangeResult.BadFlags:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_BadFlags;
                    break;
                case DisplayChangeResult.NotUpdated:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_NotUpdated;
                    break;
                case DisplayChangeResult.BadMode:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_BadMode;
                    break;
                case DisplayChangeResult.Failed:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_Failed;
                    break;
                case DisplayChangeResult.Restart:
                    msg = Properties.Settings.Default.InvalidOperation_Disp_Change_Restart;
                    break;
            }
        ...
        {
            User32.DEVMODE mode = new User32.DEVMODE();

            mode.Initialize();

            int idx = 0;

            while (User32.EnumDisplaySettings(null, idx, ref mode))
        ...
        private static DisplaySettings CreateDisplaySettingsObject(int idx, User32.DEVMODE mode)
        {
        ...
        /// </remarks>
        private static User32.DEVMODE GetDeviceMode()
        {
            User32.DEVMODE mode = new User32.DEVMODE();

            mode.Initialize();

            if (User32.EnumDisplaySettings(null, User32.ENUM_CURRENT_SETTINGS, ref mode))
                return mode;
            else
        ...
        private static string GetLastError()
        {
            ...
            if (User32.FormatMessage(User32.FORMAT_MESSAGE_FLAGS, User32.FORMAT_MESSAGE_FROM_HMODULE, (uint)err, 0, out msg, 0, 0) == 0)
                return Properties.Settings.Default.InvalidOperation_FatalError;
            else
                return msg;
        }
    }
    ...
    struct DisplaySettings
    {
        ...
        {
            return obj is DisplaySettings settings &&
